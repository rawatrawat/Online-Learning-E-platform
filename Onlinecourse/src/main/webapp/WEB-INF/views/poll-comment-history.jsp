...
<!-- Poll Comment History Content -->
<div class="container mt-4">
    <h1>Poll Comment History</h1>
    <c:if test="${not empty pollCommentHistory}">
        <table class="table">
            <thead>
            <tr>
                <th>Poll Question</th>
            ...
            <tbody>
            <c:forEach items="${pollCommentHistory}" var="pollComment">
                <tr>
                    <td>${pollComment.poll.question}</td>
                    <td>${pollComment.text}</td>
                    <td>${pollComment.timestamp}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
    </c:if>
    <c:if test="${empty pollCommentHistory}">
        <p>You have not commented on any polls yet.</p>
    </c:if>
